...
merge_stage_entry() {
    ...
    # Type-mismatch guards: mkdir -p fails if any path component along
    # DST already exists as a non-directory, and mv fails if DST exists
    # as a directory. In both cases the stage content wins silently (we
    # prefer the freshly-checked-out worktree) and we log the skip to
    # stderr so these mistakes don't rot into accumulated stage dirs.
    if [ -d "$SRC" ]; then
        if [ -e "$DST" ] && [ ! -d "$DST" ]; then
            echo "worktree-setup: stage dir '$SRC' cannot overlay existing file '$DST' — skipping" >&2
            return 0
        fi
        mkdir -p "$DST"
...
# Filter out staged paths that would recursively nest the worktree inside
# itself (e.g. $STAGE/.gc/worktrees/<rig>/<agent>/...). These accumulate
# when an agent process inadvertently writes into its own .gc/worktrees
# tree during a prior session, and each subsequent restore deepens the
# recursion until restore_stage trips over a type mismatch.
prune_self_nesting() {
    [ -n "$STAGE" ] || return 0
    RIG_BASENAME=$(basename "$RIG_ROOT")
    NEST="$STAGE/.gc/worktrees/$RIG_BASENAME/$AGENT"
    if [ -e "$NEST" ]; then
        echo "worktree-setup: pruning self-nested stage path $NEST" >&2
        rm -rf "$NEST"
        # rmdir empty parents up to .gc so we don't leave dead trees.
        rmdir "$STAGE/.gc/worktrees/$RIG_BASENAME" 2>/dev/null || true
        rmdir "$STAGE/.gc/worktrees" 2>/dev/null || true
        rmdir "$STAGE/.gc" 2>/dev/null || true
    fi
}
