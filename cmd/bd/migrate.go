...
Cross-backend migration:
  --to=postgres --dsn=…  Copy data from Dolt source to a Postgres destination.
                         Migration contract: docs/POSTGRES-BACKEND.md
                         Recovery from interrupted migration: docs/MIGRATION-RECOVERY.md
...
// exits 0:
//
//  1. Row-count parity, lossless tables. For each of the eight lossless
//     tables — issues, wisps, dependencies, wisp_dependencies, labels,
//     wisp_labels, comments, wisp_comments — the destination row count
//     equals the source row count.
//  2. Carryover present. The eight configuration tables —
//     child_counters, compaction_snapshots, config, custom_statuses,
//     custom_types, issue_counter, issue_snapshots, metadata — are
//     populated from the source. config and metadata are upserted
//     (ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value) over the
//     seed rows shipped by migration 0001; issue_counter is upserted on
//     prefix. The remaining six are CopyFrom'd over a freshly-empty
//     table.
//  3. Dependency-graph integrity. Every (issue_id, depends_on_id, type)
//     edge on the source appears with the same created_at (UTC) and
//     metadata on the destination; no edges are fabricated. Same
//     property for wisp_dependencies.
...
//     issueops.ScanIssueFrom scanner and projected via issueToPGRow.
//     Timestamps are normalized to UTC. JSONB metadata is field-by-field
//     equal to the source bytes.
//  5. Large-blob fidelity. description, notes, design,
//     acceptance_criteria, compaction_snapshots.snapshot_json, and
//     issue_snapshots.original_content are byte-identical to the source.
//  6. Audit trail surfaced, not migrated. events and wisp_events rows
//     are NOT copied. Their combined count is reported once on stderr
//     ("note: <N> audit-trail events not migrated; see
...
//     history, and never holds a write lock on it. Killing the
//     migration process mid-flight cannot leave the source in a
//     damaged state.
...
//     state on PG.
//  9. Idempotency, principled. Re-running bd migrate --to=postgres
//     against a destination that already holds bd data refuses with
//     destination_not_empty and a per-table row count. Re-running with
//     --force produces a destination indistinguishable from a single
//     fresh migration: TRUNCATE … CASCADE over every migrated table
//     happens inside the same transaction as the copy, so the
//     destination either ends in pre-existing or post-migration state
//     — never a mix.
//
...
//
// Recovery from interrupted migration: docs/MIGRATION-RECOVERY.md.
// Full contract (user-facing): docs/POSTGRES-BACKEND.md.
